    ...
    Public Class ATPRequestController
        ...
        Private Function RetrieveResults(_OracleReq As ATPRequestObj) As List(Of ATPResultObj)
            ...
            With _connection.CreateCommand()
                .CommandText = "Select MAST.session_id, MAST.COMPILE_DESIGNATOR Plan_Name, MAST.SOURCE_ORGANIZATION_CODE, " &
                    " MAST.INVENTORY_ITEM_NAME,(SELECT REPLACE (x.em_string, '.', '-') FROM apps.xxbom_cust_matching_vlvs x " &
                    " WHERE preconfigured_item_id = MAST.inventory_item_id AND x.organization_id = MAST.source_organization_id AND NVL (start_date, SYSDATE) <= SYSDATE AND NVL (end_date, SYSDATE) >= SYSDATE) Custom_Match, MAST.QUANTITY_ORDERED, MAST.UOM_CODE, XMRP.IN_REQ_DTE_TYPE , MAST.REQUESTED_DATE_QUANTITY," &
                    " MAST.AVAILABLE_QUANTITY, MAST.GROUP_SHIP_DATE, MAST.GROUP_ARRIVAL_DATE, MAST.scheduled_arrival_date," &
                    " MAST.SHIP_METHOD, MAST.demand_class, MAST.ERROR_MESSAGE, MAST.sequence_number, XMRP.IN_CUST_REQ_DATE," &
